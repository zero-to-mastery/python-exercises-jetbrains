...
class TestCase(unittest.TestCase):
    ...
    def setUp(self):
        try:
            ...
            self.fail("All 4 tests in test.py must pass. Check that you have implemented run_guess(guess, answer) function correctly.")
    ...
    def test_all_test_cases_pass(self):
        ...
        if result.errors:
            ...
            self.fail(f"Test `{failed_test}` in test.py raised an error. Check that you have implemented run_guess(guess, answer) function correctly.")

        if result.failures:
            failed_test = result.failures[0][0].id().split('.')[-1]
            self.fail(f"Test `{failed_test}` in test.py failed. Check that you have implemented run_guess(guess, answer) function correctly.")

    def test_main_guard_exists(self):
        """Check that the code uses if __name__ == '__main__' structure"""
        source = inspect.getsource(self.task)
        if "if __name__ == '__main__'" not in source:
            self.fail("Add `if __name__ == '__main__':` and move the interactive game loop there.")
